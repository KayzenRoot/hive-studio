...
4. Compile an impact map from changed paths/defects to the smallest sufficient registered harness set; reuse evidence only when exact inputs match. Include the harness registry and current checkpoint event.

## Required prompt sections
Every implementation or correction prompt carries the stable Work Order ID, a HARNESS MAP (acceptance criterion/defect → harness ID → command/budget), and these fields: OBJECTIVE; CONTEXT; SCOPE; OUT OF SCOPE; FILES/SOURCES TO READ; REQUIREMENTS; ARCHITECTURE RULES; CONSTRAINTS; ACCEPTANCE CRITERIA; TESTS and hard time budget; DELIVERABLES; REVIEW FORMAT; STOP CONDITION.
...
## Executor sequence
ANALYZE → SOURCE CHECK → WORK ORDER → CONTEXT LOCK → PREFLIGHT → EXECUTE → TARGETED HARNESS TESTS/EVIDENCE → PR → EXACT-HEAD AUDIT → CHECKPOINT EVENT/DELTA → MERGE → NEXT.
...
- Every Hive Studio project response must persist a checkpoint event per `.engineering/CHECKPOINT-PROTOCOL.md`; include its ID and GitHub link in the handoff. If no project state changed, record `NO_STATE_CHANGE` and why.
